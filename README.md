...
- Day 1–77（前三个月：第二教学月收官、第三月微观核心与成本、存货及进口报价链）：完整可学习正文，不是标题占位。
- 独立题库层（`app/data/questionBank.ts`）：与课内练习题完全分离的题目池（第 1–11 周各 14 题，共 154 题），单元测验只从题库抽题，课内练习不会漏进考场。
- 十一个单元测验：每个单元 10 道综合选择题（由题库组卷器按周抽取并确定性旋转选项位置）、1 道模拟商业案例、答案解析与掌握度。
...
目前只有前 77 天具备完整每日正文。Day 78–365 仍是课程地图与可扩展数据结构，不能把它描述成已经完成的 365 天课程。
...
`npm test` 会重新构建网站，并检查首页、77 天课程字段、独立题库 schema、十一个单元测验（只抽题库未做过的新题、选项位置分布）、52 周地图、正确率口径与复习节点。
...
├─ data/
│  ├─ lessons.ts          # Day 1–77 完整课程
│  ├─ questionBank.ts     # 独立题库种子题（qb<week>-<nn>，与课内题分离）
│  ├─ assessments.ts      # 第一至第十一单元测验（题库组卷）与案例
